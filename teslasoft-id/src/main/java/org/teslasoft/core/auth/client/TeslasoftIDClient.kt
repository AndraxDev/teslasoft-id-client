...
class TeslasoftIDClient(private val context: Activity, private val applicationSignature: String, private var apiKey: String, private var appId: String, private var settingsListener: SettingsListener?, private var syncListener: SyncListener?) {
    ...
    fun getAppSettings() {
        ...
        val url = "https://id.teslasoft.org/xauth/GetSettings.php?s=$applicationSignature&k=$apiKey&appId=$appId&token=${account["token"]}&u=${account["user_id"]}"
    ...
    fun syncAppSettings(settings: String) {
        val account: Map<String, String> = getAccount()!!
        val url = "https://id.teslasoft.org/xauth/SetSettings.php?s=$applicationSignature&k=$apiKey&appId=$appId&token=${account["token"]}&u=${account["user_id"]}&settings=$settings"
